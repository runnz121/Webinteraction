...
(() => { //이렇게 쓰는 이유는 전역 변수를 피하게 하기 위해서(지역변수가 좋음)
    ...
    function scrollLoop() { //스크롤이 얼만큼 되었는지 판별하는 함수  
        ...
        enterNewScene = false; //scene이 바뀌는 순간 playanimation 함수를 패스하게끔 해주는 변수
        prevScrollHeight = 0; //초기화 시켜주기위한 값

        for (let i = 0; i < currentScene; i++) { //현재 스크린에 나타나는 신이 i값으로 와야함
            prevScrollHeight += sceneInfo[i].scrollHeight;
        }
        //원래 delyaedYoffset이 아닌 Yoffset으로 변수 설정 하였으나, 이미지 교체시 튀는 현상 방지를 위해 변수 통일함(loop함수의)
    ...
    //부드러운 감속 처리를 위한 함수 , yOffset은 현재스크롤된위치, delayedYOffset 은 임의의 변수 지정
    function loop() {
        delayedYOffset = delayedYOffset + (yOffset - delayedYOffset) * acc;

        if(!enterNewScene) { //스크롤시 튀기는 문제를 처리하기 위해 scrolloop의 함수에서 가져옴 

            //첫번째 이미지의 부드러운 감속 처리
            const currentYOffset = delayedYOffset - prevScrollHeight;
            const values = sceneInfo[currentScene].values;
            const objs = sceneInfo[currentScene].objs;
            if (currentScene === 0 || currentScene ===2) { //currentscene 첫번째 두번째 적용
                let sequence = Math.round(calcValues(values.imageSequence, currentYOffset));

                //drawImage error 발생함(이미지가 없는 것을 불렀을시 난 경우 if문을 통해서 이미지가 있을 경우만 drawimage실행하게끔 설정)
        ...
        if (Math.abs(yOffset - delayedYOffset) < 1) { //절대값을 통해 스크롤 위, 아래 모두 감속 처리되게끔 설정
            cancelAnimationFrame(rafId); //현재 스크롤 위치와 지정 스크롤 위치 차이가 1px미만이면 rafstate false처리이고 해당 함수 정지 
            rafState = false;
        }
    }
